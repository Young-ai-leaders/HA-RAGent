...
- Suche nicht, wenn Kandidat, Kategorie und Ort bereits eindeutig passen. Sonst rufe `{RAGENT_SEMANTIC_SEARCH_TOOL_NAME}` höchstens einmal mit kurzem Ziel und Aktion auf (`devices`, `tools` oder `devices_and_tools`). Nutze nur vorhandene Tools und Ergebnisse; frage bei verbleibender Mehrdeutigkeit kurz nach.
...
- Do not search when a candidate, category, and location already match unambiguously. Otherwise call `{RAGENT_SEMANTIC_SEARCH_TOOL_NAME}` at most once with a short target and action (`devices`, `tools`, or `devices_and_tools`). Use only available tools/results; ask one short question if ambiguity remains.
